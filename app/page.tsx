...
export default async function Home() {
  ...
  return <>
    <header className="header"><h1>Golfpassi Marketing Tracker</h1><p>Nosta esiin matkat, joita ei ole hetkeen markkinoitu.</p></header>
    <main className="container">
      ...
      <div className="grid">
        {trips.map((trip) => <article className="card" key={trip.id}>
          ...
          <p className="meta">Lähtöön: {trip.days_to_start} pv · Kanavat: {trip.channels_used.join(', ') || 'ei vielä'}</p>
          <p className="reason"><strong>Syy:</strong> {priorityReason(trip)}</p>
          <div className="actions"><Link className="button" href={`/trips/${trip.id}`}>Avaa matka</Link><Link className="button secondary" href={`/actions/new?trip=${trip.id}`}>Lisää nosto</Link></div>
        </article>)}
      </div>
    </main>
  </>
