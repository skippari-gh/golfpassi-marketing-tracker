...
if (!trip) {
return ( <main className="container"> <p>Matkaa ei löytynyt.</p> </main>
)
}

const actions = await getMarketingActionsForTrip(params.id)

return ( <main className="container"> <nav className="nav"> <Link href="/">Nosta seuraavaksi</Link> <Link href="/trips">Matkat</Link>
<Link href={`/actions/new?trip=${trip.id}`}>
Lisää merkintä </Link> </nav>
